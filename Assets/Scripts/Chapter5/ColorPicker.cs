...
public class ColorPicker : MonoBehaviour {
	...
	void Awake() {
		...
		if (pickerCollider == null) {
			...
			// Make sure the collider is in the camera's frustum //平截头体
			pickerCollider.center = Vector3.zero;
            //m_camera.transform.forward世界坐标系矢量=>变换到观察空间
            //var temp = m_camera.transform.worldToLocalMatrix.MultiplyVector(m_camera.transform.forward);
            //pickerCollider.center = temp;
	...
	// Use it to grab the screen//截图
	// Note: grabing is a expensive operation//截图是很消耗性能的操作
